...
<header class="woocommerce-products-header">
	<h4 class="woocommerce-products-header__title page-title mytickets"><a href="<?php echo esc_url( wc_get_account_endpoint_url('comp-tickets') ); ?>"><?php esc_html_e( 'Active Tickets', 'competitions-for-woocommerce' ); ?></a></h4>
	<h4 class="woocommerce-products-header__title page-title mytickets active"><?php esc_html_e( 'Past Tickets', 'competitions-for-woocommerce' ); ?></h4>
</header>
...
if ( is_array( $posts_ids ) && count( $posts_ids ) > 0 ) {
	...
	foreach ( $posts_ids as $posts_id ) {
		...
		$order_history = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT distinct(ticket_number), orderid
	...
	<div class="woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
		<?php esc_html_e( 'No ticket(s) found.', 'competitions-for-woocommerce' ); ?>
	</div>
