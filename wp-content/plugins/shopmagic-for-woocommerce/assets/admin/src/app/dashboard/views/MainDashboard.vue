...
const clicks = computed(() =>
  Object.values(emails.value?.plot || {}).map(({ clicks }) => clicks)
);
const mails = computed(() =>
  Object.values(emails.value?.plot || {}).map(({ sent }) => sent)
);
const opens = computed(() =>
  Object.values(emails.value?.plot || {}).map(({ opens }) => opens)
);

const { data: posts } = originalSWRV(
  "https://shopmagic.app/wp-json/wp/v2/posts?per_page=3"
);
...
<template>
  <NLayout class="bg-transparent">
    <NLayout
      ...
      content-style="margin-bottom: 24px;"
      has-sider
    >
      <NLayoutContent
        class="bg-transparent"
        content-style="padding-right: 24px"
      >
            ...
            >
              <template #tooltip>
                ...
                <p>
                  ...
                  {{
                    __(
                      "Sum of all emails opened by their recipients.",
                      "shopmagic-for-woocommerce"
                    )
                  }}
                </p>
      ...
      <NLayoutSider :width="320" class="bg-transparent">
        <NSpace :size="24" vertical>
          <ShadyCard
            :title="__('Knowledge base', 'shopmagic-for-woocommerce')"
            size="small"
          >
            <ul>
          ...
          <ShadyCard
            :title="__('Documentation', 'shopmagic-for-woocommerce')"
            size="small"
          >
            <ul>
          ...
          <ShadyCard
            :title="__('Tip of the month 💡', 'shopmagic-for-woocommerce')"
            size="small"
          >
                ...
                },
                {
                  label: () =>
                    h(
                      ...
                      {
                        default: () =>
                          __('View subscribers', 'shopmagic-for-woocommerce'),
                      }
                    ),
                  key: 'subscribers',
                  icon: chevronIcon(),
