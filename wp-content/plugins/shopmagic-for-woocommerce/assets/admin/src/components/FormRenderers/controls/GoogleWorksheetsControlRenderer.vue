...
const spreadsheetRef = ref<string | null>(
  layout.value.data["spreadsheet"] || null
);
const worksheetRef = ref<string | null>(
  layout.value.data["spreadsheet_tab"] || null
);
...
async function getWorksheets() {
  ...
  const data = await get<string[]>(
    `/extensions/sheets/${spreadsheetRef.value}/worksheets`
  );
  ...
  if (data.value.length === 1) {
    worksheetRef.value = data.value[0];
  }
}

const message = useMessage();

async function getHeader(checked: boolean) {
  if (
    checked &&
    (spreadsheetRef.value === null || worksheetsRef.value === null)
  ) {
    message.warning(
      __(
        "Please select a spreadsheet and a worksheet first",
