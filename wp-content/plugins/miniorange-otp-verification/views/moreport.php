<div class="mo-transaction-report-container mo_report_layout">
			<form class="w-full flex flex-col items-center" method="post">
				...
				<div class="w-[75%] bg-white mt-mo-6 rounded-md">	
					<div class="mo-section-header rounded-tr-md rounded-tl-md">
						...
						<button id="mo_transaction_report" class="mo-button primary inverted" 
						<?php
						/**Disables Button for free plan.
						 *
						 * @package miniorange-otp-settings*/

						echo esc_attr( $disabled ); ?>>Generate Report</button>
					</div>
					<?php
		...
		$entries = ! is_null( $entries ) ? $entries : array();
		$hidden  = empty( $entries ) ? 'hidden' : '';
		?>
		<div class="mo_transaction_report flex justify-center" <?php echo esc_attr( $hidden ); ?>>
			<div class="w-[75%] bg-white mt-mo-6 rounded-md items-center">
				<table id="mo_report_table" class="mo-table mo-reporting-table">
					...
					foreach ( $entries as $value ) {
						?>
					<tr>
						<td> <?php echo isset( $value['phone'] ) ? esc_html( $value['phone'] ) : ''; ?></td>
						<td> <?php echo isset( $value['otp_type'] ) ? esc_html( $value['otp_type'] ) : ''; ?></td>
						<td> <?php echo isset( $value['message'] ) ? esc_html( $value['message'] ) : ''; ?></td>
						<td> <?php echo isset( $value['status'] ) ? esc_html( $value['status'] ) : ''; ?></td>
						<td> <?php echo isset( $value['time'] ) ? esc_html( $value['time'] ) : ''; ?></td>
					</tr>
					<?php } ?>
				</table>
			</div>
