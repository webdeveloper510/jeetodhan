...
if ( ! class_exists( 'WooCommerceRegistrationForm' ) ) {
	...
	class WooCommerceRegistrationForm extends FormHandler implements IFormHandler {
		...
		private function routeData() {
			...
			if ( ! array_key_exists( 'mo_wcreg_option', $_GET ) ) {
				return;
			}
			...
			$send_otp_check_option = isset( $_GET['mo_wcreg_option'] ) ? sanitize_text_field( wp_unslash( $_GET['mo_wcreg_option'] ) ) : '';
		...
		public function handle_form_options() {
			...
			$this->redirect_to_page            = isset( $_POST['page_id'] ) ? get_the_title( sanitize_text_field( wp_unslash( $_POST['page_id'] ) ) ) : 'My Account';
