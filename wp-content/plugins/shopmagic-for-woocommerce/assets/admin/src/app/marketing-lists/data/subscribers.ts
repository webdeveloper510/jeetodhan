...
export const subscribersTableColumns: DataTableColumns<ReadSubscriber> = [
  ...
  },
  {
    ...
    render: ({ email }) =>
      email || __("A customer does not exist", "shopmagic-for-woocommerce"),
  },
  {
    key: "list",
    title: () => __("List", "shopmagic-for-woocommerce"),
    render: ({ list }) =>
      list?.name || __("A list does not exists", "shopmagic-for-woocommerce"),
  },
  {
    key: "signed",
    title: () => __("Signed up", "shopmagic-for-woocommerce"),
    render: ({ active }) =>
      active ? h(NIcon, () => h(CheckmarkSharp)) : h(NIcon, () => h(Close)),
  },
  {
    key: "created",
    title: () => __("Created", "shopmagic-for-woocommerce"),
    render: ({ created }) => dayjs(created).format("D MMM, YYYY"),
