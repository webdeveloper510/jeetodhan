...
function onErrorRetry(
  revalidate: (any, opts: revalidateOptions) => void,
...
function useSWRV<Data = any, Error = any>(key: IKey): IResponse<Data, Error>;
...
function useSWRV<Data = any, Error = any>(...args): IResponse<Data, Error> {
  ...
  const revalidate = async (
    ...
    if (cacheItem) {
      const shouldRevalidate = Boolean(
        Date.now() - cacheItem.createdAt >= config.dedupingInterval ||
    ...
    const trigger = async () => {
      ...
      if (stateRef.error !== undefined) {
        const shouldRetryOnError =
          ...
          (opts ? opts.shouldRetryOnError : true);
  ...
  onUnmounted(() => {
    ...
    if (refCacheItem) {
      ...
      // refCacheItem.data = refCacheItem.data.filter(
      //   (ref) => JSON.stringify(ref) === JSON.stringify(stateRef)
      // );
    }
  });
  ...
  const res: IResponse = {
    ...toRefs(stateRef),
    mutate: (data, opts: revalidateOptions) =>
      revalidate(data, {
        ...opts,
