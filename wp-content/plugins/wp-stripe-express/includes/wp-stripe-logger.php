...
class Stripe_Express_Logger
{
  ...
  public static function createLogFile()
  {
    ...
    static::$log_file =  IT_STRIPE_EXPRESS_LOG_FOLDER . "/{$time}.log";


    //Check if directory /logs exists
    if (!file_exists(IT_STRIPE_EXPRESS_LOG_FOLDER)) {
      mkdir(IT_STRIPE_EXPRESS_LOG_FOLDER, 0777, true);
    }
  ...
  {
    if (!get_option('stripe_express_debug')) {
      return;
    }
